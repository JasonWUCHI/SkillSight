...
, duration=None, frames_length=None):
    ...
    if duration is None:
        ...
        frame_start_idx, frame_end_idx = 0, math.inf
    else:
        ...
        frame_start_idx, frame_end_idx = start_idx, end_idx
    ...
    return frames, fps, decode_all_video, frame_start_idx, frame_end_idx
...
def decode(
    ...
    try:
        if backend == "pyav":
            frames, fps, decode_all_video, frame_start_idx, frame_end_idx = pyav_decode(
                container,
                sampling_rate,
                num_frames,
                clip_idx,
    ...
    # JW: reduce the rates by linear sampling
    frames = temporal_sampling(frames, start_idx, end_idx, num_frames)

    return frames, frame_start_idx, frame_end_idx


#TODO: selective get start end

def get_start_end_idx_select(video_size, clip_size, selected_idx, num_clips):
    """
    Sample a clip of size clip_size from a video of size video_size and
    return the indices of the first and last frame of the clip. If clip_idx is
    -1, the clip is randomly sampled, otherwise uniformly split the video to
...
def pyav_decode_selective(
    container, sampling_rate, num_frames, selected_timestep, num_clips=10, target_fps=30, start=None, end=None
, duration=None, frames_length=None):
    """
    Convert the video from its original fps to the target_fps. If the video
    support selective decoding (contain decoding information in the video head),
    ...
    if duration is None:
        ...
        # selected_idx = min(selected_timestep * fps-clip_size // 2, frames_length-clip_size)
        # selected_idx = max(selected_idx, 0)
        # start_idx = selected_idx
        # end_idx = selected_idx + clip_size -1
        
        start_idx = selected_timestep[0]*fps
        end_idx = selected_timestep[1]*fps
...
def select_decode(
    ...
    try:
        if backend == "pyav":
            frames, fps, decode_all_video, frame_start_idx, frame_end_idx = pyav_decode_selective(
                container,
                sampling_rate,
                num_frames,
                selected_timestep,
