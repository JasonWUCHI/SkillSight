...
from gaze_attention import generate_attention_bias_from_gaze_coords, generate_aggregate_weight_from_gaze_coords
...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
                 drop_path=0.1, act_layer=nn.GELU, norm_layer=nn.LayerNorm, attention_type='divided_space_time', use_gaze=False):
        super().__init__()
...
class GazeEncoder(nn.Module):
    def __init__(self):
        ...
        feat_dim = 768
        self.gaze_proj = nn.Linear(15, feat_dim)
...
class SkillSightTeacherBackbone(nn.Module):
    def __init__(self, gaze_length):
        super(SkillSightTeacherBackbone, self).__init__()
        self.pretrained=True
        patch_size = 16
        self.use_gaze_attention = True
        self.crop_gaze = True  # Whether to crop gaze coordinates
        self.gaze_att_layer = "first" # Layer to apply gaze attention
        self.vision_query = 'without_time' # 'with_time' or 'without_time'
        self.att_std = 9.0  # Standard deviation for attention bias
        self.use_gaze_seq = True  # Whether to use gaze sequence features
        self.gaze_head_length = gaze_length  # Length of gaze feature vector
        self.num_cls = 4
        self.load_timesformer()
    ...
    def load_timesformer(self):
        patch_size = 16
        self.gaze_weight = nn.Parameter(torch.zeros(6), requires_grad=True)
        self.model = VisionTransformer(img_size=448, num_classes=4, patch_size=patch_size, embed_dim=768, depth=12, num_heads=12, mlp_ratio=4, qkv_bias=True, norm_layer=partial(nn.LayerNorm, eps=1e-6), drop_rate=0., attn_drop_rate=0., drop_path_rate=0.1, num_frames=16, attention_type='divided_space_time', gaze_att_weight=20.0)
        self.attention_type = 'divided_space_time'
        self.model.default_cfg = default_cfgs['vit_base_patch16_224']
        self.num_patches = (448 // patch_size) * (448 // patch_size)
        
        for param in self.model.head.parameters():
            param.requires_grad = False
     

    def load_crop_gaze_model(self):
...
class SkillSightProjectionHead(nn.Module):
    def __init__(self, in_dim, out_dim, hidden_dim=None):
        super().__init__()
        if hidden_dim is None:
            hidden_dim = (in_dim + out_dim) // 2  # simple heuristic
...
class SkillSightTeacherFeatureExtractor(nn.Module):
    def __init__(self, model_path, gaze_length):
        super(SkillSightTeacherFeatureExtractor, self).__init__()
    ...
    def load_distill_model(self):
        ...
        self.teacher_model = SkillSightTeacherBackbone(self.gaze_length)
            
        checkpoint = torch.load(self.model_path, map_location="cpu")  # or "cuda" if using GPU
        state_dict = checkpoint["model_state"]
        # Remove classifier-related weights that don’t match your current model
        exclude = ["model.head.weight", "model.head.bias", "fc2.weight", "fc2.bias"]
        for k in exclude:
            if k in state_dict:
                print(f"Excluding {k}")
                del state_dict[k]
        ...
        # self.student_proj = SkillSightProjectionHead(768,256)
        # self.teacher_proj = SkillSightProjectionHead(in_dim=768*3, out_dim=256)

    def forward(self, x, x_cropped, gaze, scenario_id):
   
        teacher_times, teacher_gaze, teacher_crop, teacher_pred = self.teacher_model.get_forward_features(x, x_cropped, gaze, scenario_id)  # [B, 4]
        teacher_feat = torch.cat([teacher_times, teacher_gaze, teacher_crop], dim=-1)  # Concatenate features
        
        return teacher_feat



class SkillSightStudent(nn.Module):
    def __init__(self, input_dim=21, d_model=128, nhead=4, num_layers=2, num_classes=4, max_len=160):
        super().__init__()
        self.cls_token = nn.Parameter(torch.zeros(1, 1, d_model))
        self.tks_token = nn.Parameter(torch.zeros(1, 1, d_model))
        ...
        pos_embed = self._build_sinusoidal_positional_encoding(max_len + 3, d_model)  # +2 for cls + distill
        self.register_buffer("pos_embed", pos_embed, persistent=False)

    def _build_sinusoidal_positional_encoding(self, length, d_model):
    ...
    def forward(self, x, teacher_feat=None, training=False):
        """
        x: (B, T, D)
        """
        B = x.size(0)
        # x = torch.cat([x[:,:,:13], x[:,:,15:]], dim=-1)
        x = self.embed(x)   # (B, T, d_model)

        # prepend CLS token
        cls_tokens = self.cls_token.expand(B, -1, -1)  # (B, 1, d_model)
        tks_tokens = self.tks_token.expand(B, -1, -1)  # (B, 1, d_model)
        dis_tokens = self.dis_token.expand(B, -1, -1)
        x = torch.cat([cls_tokens, tks_tokens, dis_tokens, x], dim=1)          # (B, T+2, d_model)
        ...
        cls_out = x[:, 0]      
        tks_out = x[:, 1]                        # (B, d_model)
        
        if training:
            dis_out = self.student_proj(x[:, 2])                             # (B, d_model)
            target_feat = self.teacher_proj(teacher_feat)
        else:
            dis_out = torch.zeros(1)
            target_feat = torch.zeros(1)

        return dis_out, target_feat, self.fc(cls_out), self.fc2(tks_out) 